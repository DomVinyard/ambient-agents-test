import { Box, VStack, HStack, Text, List, ListItem, Badge, Flex, Menu, MenuButton, MenuList, MenuItem, Button, Divider } from '@chakra-ui/react';
...
export default function FileList({ files, selectedFile, onFileSelect, onDeleteFile, onDeleteAll, onCompileProfile, isCompiling }: FileListProps) {
  ...
  const compiledFiles = fileEntries.filter(([fileName]) => {
    ...
    return compiledCategories.indexOf(categoryA) - compiledCategories.indexOf(categoryB);
  });
  
  const generatedFiles = fileEntries.filter(([fileName]) => {
    const category = getCategory(fileName);
    return generatedCategories.includes(category);
  }).sort(([fileNameA], [fileNameB]) => {
    const categoryA = getCategory(fileNameA);
    const categoryB = getCategory(fileNameB);
    return generatedCategories.indexOf(categoryA) - generatedCategories.indexOf(categoryB);
  });
