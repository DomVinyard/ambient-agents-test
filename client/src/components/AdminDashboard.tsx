...
export default function AdminDashboard({ onLogout }: AdminDashboardProps) {
  ...
  const [insightsByEmail, setInsightsByEmail] = useState<
  ...
  const handleBuildProfile = async (
    ...
    setMasterProgress((prev) => ({
      ...prev,
      fetchProgress: prev.isEndToEndProcess
        ? { processed: 1, total: 1 }
        : prev.fetchProgress, // Mark fetch complete
    ...
    try {
      ...
      setMasterProgress((prev) => ({
        ...prev,
        insightsProgress: {
          processed: currentEmails.length,
          total: currentEmails.length,
        }, // Mark complete
      ...
      const applyPromises = Object.entries(insightsByCategory).map(
        async ([category, categoryInsights]: [string, any[]]) => {
          ...
          try {
            const response = await axios.post(
              "http://localhost:3001/api/ai/blend-profile",
              {
                tokens,
                category,
            ...
            setMasterProgress((prev) => ({
              ...prev,
              profileProgress: {
                processed: profileProcessedCount,
                total: categoryCount,
              },
            }));
          }
      ...
      const resultStatus =
        currentErrorCount + profileErrorCount > 0 ? "warning" : "success";
  ...
  const handleDeleteAllData = async () => {
    if (
      window.confirm(
        "Are you sure you want to delete all bio and email data? This action cannot be undone."
      )
    ) {
      try {
        // Clear IndexedDB
        ...
        toast({
          ...
          description:
            "All emails, insights, and profile files have been deleted.",
          status: "success",
          duration: 3000,
          isClosable: true,
        });
  ...
  const handlePreviewFile = (fileName: string, content: string) => {
    setPreviewModal({
      isOpen: true,
      fileName,
      content,
    ...
    <Box h="100vh" bg="gray.50">
      ...
      <Flex h="calc(100vh - 60px)" w="100%">
        ...
        (insights.length > 0 ||
          extractingInsights ||
          insightError ||
          (selectedEmailId && selectedEmailId in insightsByEmail)) ? (
          <Box w="20%" h="100%">
            <InsightsViewer
              insights={insights}
        ...
        {Object.keys(files).length > 0 && (
          <Box w="26.67%" h="100%">
            <FileEditor file={selectedFileItem} onSave={handleSaveFile} />
          </Box>
        )}
      </Flex>
