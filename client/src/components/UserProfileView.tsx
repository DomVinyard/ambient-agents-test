...
export default function UserProfileView({
  ...
  const updateStatusMessage = (message: string, source: string) => {
    console.log(
      `🔄 STATUS CHANGE [${source}]: "${statusMessage}" → "${message}"`
    );
  ...
  const calculateOverallProgress = () => {
    ...
    if (masterProgress.fetchProgress) {
      const fetchPercent =
        (masterProgress.fetchProgress.processed /
          masterProgress.fetchProgress.total) *
        10;
    ...
    if (masterProgress.insightsProgress) {
      const insightsPercent =
        (masterProgress.insightsProgress.processed /
          masterProgress.insightsProgress.total) *
        70;
    ...
    if (masterProgress.profileProgress) {
      const profilePercent =
        (masterProgress.profileProgress.processed /
          masterProgress.profileProgress.total) *
        10;
  ...
  useEffect(() => {
    console.log(
      `🔄 PROGRESS STATUS EFFECT: isBuilding=${isBuilding}, progress=${calculateOverallProgress()}%`
    );
  ...
  useEffect(() => {
    ...
    const processQueue = async () => {
      ...
      if (currentProgress >= 80) {
        console.log(
          `🚫 QUEUE SKIP - past insights phase (progress: ${currentProgress}%)`
        );
        return;
      }

      console.log(
        `⏰ QUEUE PROCESSOR: Queue length: ${currentQueue.length}, hasShownInitial: ${currentHasShownInitial}, messageCount: ${messageCount}`
      );

      // First show initial message when entering insights phase
      if (
        currentQueue.length === 0 &&
        !currentHasShownInitial &&
        messageCount === 0
      ) {
        messageCount++;
      ...
      if (currentQueue.length > 0) {
        console.log(
          `🔥 PROCESSING INSIGHT FROM QUEUE: ${currentQueue.length} insights available`
        );
        ...
        console.log(
          `🗑️ REMOVING ITEM FROM QUEUE: ${currentQueue.length} → ${newQueue.length} insights`
        );
        ...
        const statusMessage =
          queueItem.statusMessage || "Discovering things about you...";
  ...
  useEffect(() => {
    ...
    if (fakeProgressFrom90 > 0 && fakeProgressFrom90 < 10) {
      console.log(
        `🎭 CONTINUING FAKE PROGRESS: currently at ${90 + fakeProgressFrom90}%`
      );
    ...
    try {
      ...
      const result = await profileService.buildProfile({
        ...
        onInsightReceived: async (insights: any[]) => {
          ...
          console.log(
            `🔥 RECEIVED ${insights.length} insights:`,
            insights.slice(0, 2).map((i) => i.insight)
          );
          ...
          for (let i = 0; i < insights.length; i += BATCH_SIZE) {
            ...
            const batchPromise = (async () => {
              try {
                console.log(
                  `🤖 SENDING BATCH TO AI: ${batch.length} insights to pick from`
                );
                const response = await axios.post(
                  "http://localhost:3001/api/ai/generate-status",
                  {
                    insights: batch,
                    userInfo,
                  }
          ...
          setInsightQueue((prev: any[]) => {
            ...
            const totalProcessed = batchResults.reduce(
              (sum, result) => sum + result.batchSize,
              0
            );
            console.log(
              `📝 QUEUE UPDATED: ${prev.length} → ${newQueue.length} best insights (from ${totalProcessed} total insights)`
            );
            return newQueue;
          });
        },
      ...
      const existingFiles =
        storageService.getItem("ambient-agents-files") || {};
      const updatedFiles = {
        ...existingFiles,
        "full.md": {
          name: "full.md",
  ...
  const handleContentChange = (value: string = "") => {
    ...
    const updatedFiles = {
      ...
      "full.md": {
        ...
        content: value,
        createdAt:
          existingFiles["full.md"]?.createdAt || new Date().toISOString(),
        updatedAt: new Date().toISOString(),
      },
    };
    storageService.setItem("ambient-agents-files", updatedFiles);
  ...
  const isBuildingComplete =
    masterProgress.compileProgress?.processed ===
      masterProgress.compileProgress?.total &&
  ...
  const handleMasterProgressUpdate = useCallback(
    (stage: string, progress: { processed: number; total: number } | null) => {
